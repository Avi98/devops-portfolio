...
import { forwardRef, HTMLDevUIProps } from "@/utils/system"
...
export type ButtonProps = Props &
  Omit<HTMLDevUIProps<"button">, keyof VariantProps> &
  VariantProps
...
const Button = forwardRef<ButtonProps, "button">(
  (
    ...
    const handleDrip = (
      e: React.MouseEvent<HTMLButtonElement> | PressEvent | Event
    ) => {
    ...
    return (
      <StyledButton
        ...
        className={clsx("devui-button", `devui-button--${getState}`, className)}
        css={{
          ...(css as any),
          ...cssColors,
        }}
      ...
      >
        {React.Children.count(children) === 0 ? (
          ...
          <>
            ...
            <div
              className={clsx("devui-button-text", {
                "devui-button-text-right": isRight,
                "devui-button-text-left": !isRight,
              })}
            >
              {children}
            </div>
          ...
          <span className="devui-button-text">{children}</span>
        )}
        <ButtonDrip
          className="devui-button-drip"
          color="white"
          {...dripBindings}
        />
      </StyledButton>
...
if (__DEV__) {
  Button.displayName = "DevUI.Button"
}

Button.toString = () => ".devui-button"
